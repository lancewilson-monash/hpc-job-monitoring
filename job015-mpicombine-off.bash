...
# ########################################
mpirun `which relion_refine_mpi` --o Class2D/job015-$LOGDATE/run --i Select/job014/particles.star  --preread_images  --pool 30 --pad 2  --ctf  --iter 25 --tau2_fudge 2 --particle_diameter 200 --K 100 --flatten_solvent  --zero_mask  --oversampling 1 --psi_step 12 --offset_range 5 --offset_step 2 --norm --scale  --j 1 --gpu "" | sed 's/\r/\n/g' | ts %.s | tee job-$LOGDATE.log &
# ########################################
PID1=$!
#echo $PID1
